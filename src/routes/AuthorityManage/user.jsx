...
@connect(({ authorityManageUser, loading }) => ({
  authorityManageUser,
  loading: loading.models.authorityManageUser,
}))
@Form.create()
export default class TableList extends PureComponent {
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'authorityManageUser/fetch',
    });
  }
  // 表格改变事件
  ...
  handleMenuClick = e => {
    ...
    switch (e.key) {
      case 'remove':
        dispatch({
          type: 'authorityManageUser/remove',
          payload: {
  ...
  handleModalVisible = (flag, selectedKey) => {
    ...
    if (modalVisible && selectedKey) {
      dispatch({
        type: 'authorityManageUser/fetchbyId',
        payload: { key: selectedKey },
      });
    } else if (Boolean(modalVisible) === false) {
      dispatch({
        type: 'authorityManageUser/clearUserInfo',
        payload: {},
      });
    }
    this.setState({
  ...
  handleAdd = fields => {
    this.props.dispatch({
      type: 'authorityManageUser/addOrUpdate',
      payload: {
        addData: fields,
      },
    });
  ...
  render() {
    const { authorityManageUser: { data, userInfo }, loading } = this.props;
    ...
    const parentMethods = {
      ...
      modalVisible,
      userInfo,
      title: userInfo == null ? '新增用户信息' : '修改用户信息',
      loading,
    };
